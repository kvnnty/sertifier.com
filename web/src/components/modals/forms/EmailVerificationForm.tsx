...
export default function EmailVerificationForm({ email }: EmailVerificationFormProps) {
  ...
  const handleResend = async () => {
    ...
    setCanResend(false);
    try {
      const response = await axiosClient.post("/auth/resend-otp", { email, purpose: "email_verification" });
      setTimer(30);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await axiosClient.post("/auth/verify-otp", { email, code: otp, purpose: "email_verification" });
